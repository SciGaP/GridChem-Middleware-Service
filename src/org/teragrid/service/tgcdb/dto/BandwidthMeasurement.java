...
public class BandwidthMeasurement implements TgcdbDTO {
	...
	/**
	 * @param fromResourceName the fromResourceName to set
	 */
	...
	/**
	 * @param toResourceID the toResourceID to set
	 */
	...
	/**
	 * @param toResourceName the toResourceName to set
	 */
	...
	/**
	 * @param lastMeasured the lastMeasured to set
	 */
	...
	/**
	 * @param megabytespersecond the megabytespersecond to set
	 */
	...
	public boolean equals(Object o) {
		if (o instanceof String) {
			...
			return (fromResourceID.equals(((BandwidthMeasurement)o).fromResourceID) && 
					toResourceID.equals(((BandwidthMeasurement)o).toResourceID));
		} else if (o instanceof ComputeDTO) {
			return toResourceID.equals(((ComputeDTO)o).getId());
		}
		return false;
	}
	...
	public String toHtml() {
		return "<tr>" + 
			formatColumn("<a href=\"http://info.teragrid.org/web-apps/html/ctss-resources-v1/ResourceID/" + fromResourceID + "\">" + fromResourceID + "</a>") + 
			formatColumn(fromResourceName) + 
			formatColumn(fromPath) + 
			formatColumn("<a href=\"http://info.teragrid.org/web-apps/html/ctss-resources-v1/ResourceID/" + toResourceID + "\">" + toResourceID + "</a>") +  
			formatColumn(toResourceName) +
			formatColumn(toPath) + formatColumn(lastMeasured) + 
			formatColumn(megabytespersecond) +"</tr>";
	}
	...
	public String toJSONString() {
		...
		try {
			...
			.object().key("fromResourceID").value(fromResourceID).endObject()
			.object().key("fromResourceName").value(fromResourceName).endObject()
			.object().key("toResourceID").value(toResourceID).endObject()
			.object().key("toResourceName").value(toResourceName).endObject()
			.object().key("toPath").value(toPath).endObject()
			.object().key("lastMeasured").value(lastMeasured).endObject()
			.object().key("megabytespersecond").value(megabytespersecond).endObject()
			.endObject();
