...
//	}
	...
	public String toString() {
		return quote(id) + "," + quote(username) + "," +
				...
				quote(position) + "," + quote(projectId) + ",";
	}
	...
	public String toCsv() {
		return quote(id) + "," + quote(username) + "," +
			quote(firstName) + "," + quote(middleName) + "," +
			quote(lastName) + "," + quote(email) + "," +
			address.toCsv() + "," + quote(busPhoneNumber) + "," +
			quote(busPhoneExtension) + "," + quote(faxNumber) + "," +
			quote(organization) + "," + quote(department) + "," +
			quote(position) + "," + quote(projectId);
	}
	...
	public String toHtml(String url) {
		String path = url.substring(0,url.indexOf("profile-v1")+"profile-v1".length());
		
		return "<tr>" + formatColumn(id) + 
			//formatColumn("<a href=\"" + path + "/username/" + username + "\">" + username + "</a>") +
			...
			formatColumn("<a href=\"" + path + "/username/" + username + "/project/project_number/" + projectId + "\">" + projectId + "</a>") + 
			"</tr>";
	}
	...
	public String toJSONString() {
		...
		try {
			js.object().key("dn")
			.object().key("id").value(this.id).endObject()
			.object().key("username").value(this.username).endObject()
			.object().key("firstName").value(this.firstName).endObject()
			.object().key("middleName").value(this.middleName).endObject()
			.object().key("lastName").value(this.lastName).endObject()
			.object().key("email").value(this.email).endObject()
			.object().key("address").value(this.address).endObject()
			.object().key("homePhoneNumber").value(this.homePhoneNumber).endObject()
			.object().key("homePhoneExtension").value(this.homePhoneExtension).endObject()
			.object().key("busPhoneNumber").value(this.busPhoneNumber).endObject()
			.object().key("busPhoneExtension").value(this.busPhoneExtension).endObject()
			.object().key("faxNumber").value(this.faxNumber).endObject()
			.object().key("organization").value(this.organization).endObject()
			.object().key("department").value(this.department).endObject()
			.object().key("position").value(this.position).endObject()
			.object().key("projectId").value(this.projectId).endObject()
			.endObject();
	...
	public String toPerl() {
		...
		return " {\n" + 
				...
				"   email => '" + email + "',\n" + 
				address.toPerl() +  
				"   business.phone => '" + busPhoneNumber + "',\n" + 
				"   business.phone.ext => '" + busPhoneExtension + "',\n" + 
				"   business.fax => '" + faxNumber + "',\n" + 
