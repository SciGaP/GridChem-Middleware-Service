...
//	}
	...
	public String toJSONString() {
		...
		try {
			js.object().key("allocation")
			.object().key("id").value(this.allocationId).endObject()
			.object().key("project.id").value(this.projectId).endObject()
			.object().key("project.title").value(this.projectTitle).endObject()
			.object().key("start.date").value(TGUtil.formatUTC(startDate)).endObject()
			.object().key("end.date").value(TGUtil.formatUTC(endDate)).endObject()
			.object().key("base.allocation").value(this.baseAllocation).endObject()
			.object().key("used.allocation").value(this.usedAllocation).endObject()
			.object().key("remaining.allocation").value(this.remainingAllocation).endObject()
			.object().key("resource.name").value(this.allocResourceName).endObject()
			.object().key("pi.first.name").value(this.piFirstName).endObject()
			.object().key("pi.last.name").value(this.piLastName).endObject()
			.object().key("account.state").value(this.acctState).endObject()
			.object().key("project.state").value(this.projState).endObject()
			.endObject();
	...
	public String toCsv() {
		return quote(allocationId) + "," + quote(projectId) + "," +
			quote(projectTitle) + "," + quote(TGUtil.formatUTC(startDate)) + "," +
			quote(TGUtil.formatUTC(endDate)) + "," + quote(baseAllocation) + "," +
			quote(usedAllocation) + "," + quote(remainingAllocation) + "," +
			quote(allocResourceName) + "," + quote(piFirstName) + "," +
			quote(piLastName) + "," + quote(acctState) + "," +
			quote(projState);
	}
	...
	public String toHtml() {
		...
		formatColumn(projectId) +
		formatColumn(projectTitle) + 
		formatColumn(TGUtil.formatUTC(startDate)) +
		formatColumn(TGUtil.formatUTC(endDate)) + 
		formatColumn(baseAllocation) +
		formatColumn(usedAllocation) + 
		formatColumn(remainingAllocation) +
		formatColumn(allocResourceName) + 
		formatColumn(piFirstName) +
		formatColumn(piLastName) + 
		formatColumn(acctState) +
...
//		ComputeDTO system = ResourceCache.getResourceByTgcdbName(allocResourceName);
//    	if (system == null) {
...
//    	}
		return "<tr>" + 
		formatColumn(allocationId) + 
		formatColumn("<a href=\"" + path + "/project/project_number/" + projectId + "\">" + projectId + "</a>") +
		formatColumn(projectTitle) + 
		formatColumn(TGUtil.formatUTC(startDate)) +
		formatColumn(TGUtil.formatUTC(endDate)) + 
		formatColumn(baseAllocation) +
