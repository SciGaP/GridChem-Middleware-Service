...
public class JobDTO implements TgcdbDTO {
	...
	/**
	 * @param localJobId the localJobId to set
	 */
	...
	/**
	 * @param projetct the projetct to set
	 */
	...
	public String toHtml() {
		...
		formatColumn(charge) + formatColumn(nodeCount) +
		formatColumn(processorCount) + formatColumn(queue) +
	...
	public String toHtml(String url) {
		...
		formatColumn(TGUtil.formatUTC(startTime)) + 
		formatColumn(TGUtil.formatUTC(endTime)) +
		formatColumn(localJobId) + formatColumn("<a href=\"" + path + "/resource/resourceid/" + resourceID + "\">" + resourceID + "</a>") +
		formatColumn("<a href=\"" + path + "/project/project_number/" + projectNumber + "\">" + projectNumber + "</a>")+"</tr>";
	}
	...
	public String toJSONString() {
		...
		try {
			...
			.object().key("wall.time").value(this.wallTime).endObject()
			.object().key("charge").value(this.charge).endObject()
			.object().key("node.count").value(this.nodeCount).endObject()
			.object().key("proc.count").value(this.processorCount).endObject()
			.object().key("queue.name").value(this.queue).endObject()
			.object().key("start.time").value(TGUtil.formatUTC(startTime)).endObject()
			.object().key("end.time").value(TGUtil.formatUTC(endTime)).endObject()
			.object().key("local.id").value(this.localJobId).endObject()
			.object().key("ResourceID").value(this.resourceID).endObject()
			.object().key("project_number").value(this.projectNumber).endObject()
			.endObject();
